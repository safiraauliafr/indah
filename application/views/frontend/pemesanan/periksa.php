        ...
        <div class="col-md-12">
            ...
            <div class="form-group">
                <div class="col-sm-offset-2 col-sm-6">
                    <textarea class="form-control" id="alamat" name="alamat" placeholder="Alamat pengiriman"
                        maxlength="255"
                        required><?= set_value('alamat', htmlspecialchars($pengguna->alamat)) ?></textarea>
                    <span class="help-block">Kunjungi halaman <?= anchor_popup('akun', 'pengaturan akun') ?> untuk
                        mengatur alamat utama secara permanen</span>
                </div>
                <div class="col-sm-4">
                    <address>
                        ...
                        <?= $pengguna->nomor_telepon ?><br>
                        <abbr title="Surel"><i class="fa fa-envelope" aria-hidden="true"></i></abbr>
                        <?= $pengguna->nama ?>
                    </address>
                </div>
            </div>
            <div class="form-group">
                ...
                <div class="col-sm-10">
                    <select class="form-control" name="kodepos" id="kodePos">
                        <option value="" disabled selected hidden>Kode Pos</option>
                    </select>
                </div>
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
                ...
                <div class="col-sm-10">
                    <h4><?= $totaljenis ?> jenis, <?= $this->cart->total_items() ?> unit, <?= ceil($totalberat) ?> <abbr
                            title="kilogram">kg</abbr> <small>(dibulatkan ke atas dari berat mula <?= $totalberat ?> kg
                            untuk pengiriman)</small></h4>
                </div>
            </div>
            <div class="form-group">
            ...
            <input type="hidden" name="total_biaya_pembelian" id="total_biaya_pembelian"
            ...
            <div class="btn-group" role="group" aria-label="Kontrol Keranjang">
                <?=
                        ...
                        ?>
                <button class="btn btn-primary btn-lg" type="submit"><i class="fa fa-money" aria-hidden="true"></i>
            ...
            function promptDeleteAll() {
                return confirm("Apakah anda yakin ingin menghapus semua " + < ? = $this - > cart - > total_items() ? > +
                    " barang dari keranjang belanja anda?");
            }
            </script>
            <?php
            } else {
                ?>
            <div class="alert alert-warning">Anda belum memiliki barang di keranjang belanja anda,
                <?= anchor('produk', 'silahkan pilih barang belanjaan terlebih dahulu') ?>.</div>
...
function getKota(idpro) {
    ...
    $.getJSON("pemesanan/kota/" + idpro, function(data) {
        $.each(data, function(i, field) {
            $op.append('<option value="' + field.city_id + '">' + field.type + ' ' +
                field
                .city_name +
                '</option>');
        });
...
function getKodepos(idkota) {
    var $op = $("#kodePos");
    $('.opsi').remove();
    $.getJSON("pemesanan/kodepos/" + idkota, function(data) {
        var result = Object.entries(data);
        // console.log(result);
        $op.append('<option class="opsi" value="' + result[5][1] +
            '" selected="selected" disabled>' +
            result[5][1] +
            '</option>');
        $op.append('<input name="kodepos" class="opsi" type="text" value="' + result[5][1] +
            '">');
    });
}
    ...
    $.getJSON("pemesanan/tarif/" + origin + "/" + des + "/" + qty + "/" + cour, function(data) {
        $.each(data, function(i, field) {
            ...
            for (var i = 0; i < tarif.length; i++) {
                var opt = service[i] + " | " + " Rp. " + tarif[i] + " | " + waktu[i] +
                    " Hari";
